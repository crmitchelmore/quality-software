...
export function buildDetectors(profile: ResolvedProfile): Detector[] {
  ...
  if (boundaryPattern) {
    ...
    const domainGlobs = (opts.domainGlobs as string[]) ?? ["src/domain/**", "src/application/**"];
    const forbidImportsFrom = (opts.forbidImportsFrom as string[]) ?? ["src/infrastructure/**"];
    detectors.push(
      forbiddenImportDetector({ patternId: boundaryPattern.id, domainGlobs, forbidImportsFrom }),
    );
  }
